...
BEGIN
  SELECT
    ...
    IFNULL(`ie`.`slot_n1`, 0) AS `ensoul_slot_n1`,
    IFNULL(`ie`.`slot_n2`, 0) AS `ensoul_slot_n2`,
    IFNULL(`ie`.`slot_bm`, 0) AS `ensoul_slot_bm`,
...
BEGIN
  ...
  DELETE FROM `items_ensoul` WHERE `items_ensoul`.`item_id` = iItemID LIMIT 1;
END $$
...
CREATE PROCEDURE `lip_StoreItem` (IN iItemID INTEGER UNSIGNED,
                                 ...
                                 IN iEnsoulSlotN1 SMALLINT UNSIGNED,
                                 IN iEnsoulSlotN2 SMALLINT UNSIGNED,
                                 IN iEnsoulSlotBm SMALLINT UNSIGNED,
                                 IN iBlessed INTEGER,
                                 IN iDamaged INTEGER,
  ...
  CASE
    WHEN iRowCount = 1 THEN -- insert new
      BEGIN
        ...
        IF (iEnsoulSlotN1 > 0) OR (iEnsoulSlotN2 > 0) OR (iEnsoulSlotBm > 0) THEN -- have ensoul in a new item
            INSERT LOW_PRIORITY `items_ensoul` (
                `item_id`,
                `slot_n1`,
                `slot_n2`,
                `slot_bm`
                ...
                `slot_n1` = iEnsoulSlotN1,
                `slot_n2` = iEnsoulSlotN2,
                `slot_bm` = iEnsoulSlotBm;
            -- SELECT 'lip_StoreItem[443]', iItemID, iEnsoulSlotN1, iEnsoulSlotN2, iEnsoulSlotBm, ROW_COUNT();
        END IF;
...
--        SELECT 'lip_StoreItem[407]', iItemID, iDuration, ROW_COUNT();
        ...
        IF (iVariStat1 > 0) OR (iVariStat2 > 0) THEN -- have some new variation(augumentation) in existing item
          INSERT LOW_PRIORITY INTO `items_variation` (
            ...
            `stat1` = iVariStat1,
            `stat2` = iVariStat2;
          -- SELECT 'lip_StoreItem[462]', iItemID, iVariStat1, iVariStat2, ROW_COUNT();
        ...
        IF (iEnsoulSlotN1 > 0) OR (iEnsoulSlotN2 > 0) OR (iEnsoulSlotBm > 0) THEN -- have new ensoul values set for an existing item
            INSERT LOW_PRIORITY INTO `items_ensoul` (
                `item_id`,
                `slot_n1`,
                `slot_n2`,
                `slot_bm`
                ...
                `slot_n2` = iEnsoulSlotN2,
                `slot_bm` = iEnsoulSlotBm;
            -- SELECT 'lip_StoreItem[571]', iItemID, iEnsoulSlotN1, iEnsoulSlotN2, iEnsoulSlotBm, ROW_COUNT();
        ELSE -- delete ensoul (ensoul removed or have not been set)
            DELETE LOW_PRIORITY FROM `items_ensoul` WHERE `item_id` = iItemID LIMIT 1;
            -- SELECT 'lip_StoreItem[574]', iItemID, iEnsoulSlotN1, iEnsoulSlotN2, iEnsoulSlotBm, ROW_COUNT();
        END IF;
