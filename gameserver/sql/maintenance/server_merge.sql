...
INSERT INTO src_db.objectIdx (object_id)
SELECT id FROM src_db.pawnshop;
INSERT INTO src_db.objectIdx (object_id)
...
SELECT object_id,shortIdx+(SELECT GREATEST(MAX(l.object_id), MAX(r.object_id)) FROM dst_db.objectIdx l, src_db.objectIdx r)
from src_db.objectIdx;

-- cleanup
delete from src_db.character_quests where char_id not in (select obj_id from src_db.characters);
delete from src_db.character_skills where char_obj_id not in (select obj_id from src_db.characters);
delete from src_db.pawnshop where ownerId not in (select obj_id from src_db.characters);
...
UPDATE src_db.pawnshop a,src_db.exportIdx b
SET a.id = b.new_id where a.id = b.old_id;
...
UPDATE src_db.ally_data a,src_db.exportIdx b
SET a.leader_id = b.new_id where a.leader_id = b.old_id;

UPDATE src_db.pets a,src_db.exportIdx b
SET a.objId = b.new_id where a.objId = b.old_id;

UPDATE src_db.oly_nobles a,src_db.exportIdx b
SET a.char_id = b.new_id where a.char_id = b.old_id;
...
UPDATE src_db.pawnshop a,src_db.exportIdx b
SET a.ownerId = b.new_id
WHERE a.ownerId= b.old_id;
...
insert into dst_db.accounts(login, `password`, l2email)
select src_db.acc_conv.dst_login, x.password, x.l2email
...
SELECT 'items ok.';
INSERT INTO dst_db.pawnshop SELECT * FROM src_db.pawnshop;
SELECT 'pawnshop ok.';
